    ...
    : BaseService<KtxPhong>(unitOfWork, phongRepository), IPhongKtxService
{
    public override async Task<Result<KtxPhong>> CreateAsync(KtxPhong entity)
    {
        try
        {
            ...
            {
                return new Result<KtxPhong>(new BadRequestException($"Mã phòng '{entity.MaPhong}' đã tồn tại."));
            }
            if (entity.SoLuongGiuong > 0)
            {
                if (entity.Id == Guid.Empty) entity.Id = Guid.NewGuid();
                entity.GiuongKtxs ??= new List<KtxGiuong>();
    ...
    public override async Task<Result<KtxPhong>> UpdateAsync(Guid id, KtxPhong entity)
    {
        try
        {
            ...
            if (existingPhong == null)
                return new Result<KtxPhong>(new NotFoundException("Không tìm thấy phòng"));

            if (entity.SoLuongGiuong < existingPhong.SoLuongDaO)
                return new Result<KtxPhong>(new BadRequestException(
